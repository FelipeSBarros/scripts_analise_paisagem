...
#writing tables with the IDHEX in the name of the text file
  write.table(nodes[[i]], file=sprintf("%s.nodes_oldgrowth.txt",shapes[[i]]$HEXID[i]), row.names = F, col.names = F,    sep="\t", quote=F)
...
#writting distance results in text files
  write.table(distancia[[i]],file=sprintf("%s.dists_oldgrowth.txt", shapes[[i]]$HEXID[i]),row.names=F, col.names=F, sep="\t", quote=F)
    print(i)
}, gcFirst = TRUE)
############
